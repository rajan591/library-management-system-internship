...
if (isset($_POST['btnUpdate'])) {
	...
	if (count($err) == 0) {
		...
		//make query to insert data
		 $sql = "update books set name='$name',authors='$authors',edition='$edition',status='$status',quantity='$quantity', department =  '$department' where bid=$id ";
		//execute query
		$db->query($sql);
...
$book = $result->fetch_assoc();
 ...
 <div class="container">	
 		...
 		<form action="<?php echo $_SERVER['PHP_SELF'] ?>?bid=<?php echo $id ?>" method="post">
	 	 ...
	 	 <input type="text" name="department" class="form-control" id="department" placeholder="Enter department" value="<?php echo $book['department'] ?>" >
	 	  <?php 
	 	  ...
	 	  ?>	 	 
	 	 <br>
	 	 <input type="submit" name="btnUpdate" class="btn btn-success" value="Update Category">
	 </form>
 	</div>
 	</div>
 	</div>
