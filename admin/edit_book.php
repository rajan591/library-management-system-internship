...
if (isset($_POST['btnUpdate'])) {
	...
	if (count($err) == 0) {
		require "connection.php";
		 echo $sql = "update books set name='$name',authors='$authors',edition='$edition',status='$status',quantity='$quantity' where bid=$id ";
		//execute query
		$db -> query($sql);
...
$book = $result->fetch_assoc();
 ...
 <div class="container">	
 		...
 		<form action="<?php echo $_SERVER['PHP_SELF'] ?>?bid=<?php echo $id ?>" method="post">
	 	 ...
	 	 <input type="number" name="department" class="form-control" id="department" placeholder="Enter department" value="<?php echo $book['DepartmentID'] ?>" >
	 	  <?php 
